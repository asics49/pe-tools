...
refs = con.execute(
    "SELECT stable_id, local_title FROM items WHERE local_title='參考資料' AND is_folder=1 AND is_tombstone=0"
).fetchall()

print(f"找到 {len(refs)} 個「參考資料」資料夾\n")

for r_sid, _ in refs:
    r_cid = cloud_id(r_sid)
    parents = get_parents(r_sid)
    if not parents: continue
    sub_sid = parents[0]
    sub_title = get_title(sub_sid)
    gp_list = get_parents(sub_sid)
    gp_title = get_title(gp_list[0]) if gp_list else '?'
    url = f'https://drive.google.com/drive/folders/{r_cid}'
