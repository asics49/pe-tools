...
| 檔案 | 說明 |
|------|------|
| `index.html` | 評鑑文件總覽，89 筆文件，含篩選、模組卡片列（13 張，可捲動+左右箭頭）、三色按鈕、進度備份匯出/匯入 |
| `pe-class-tools.html` | 9 模組合併頁，由 build.py 產生 |
| `guide.html` | 工具使用說明網頁，含左側導覽列、折疊 Q&A、手機響應式（模組說明已更新至 9 個模組） |
...
模組卡片列（頁面頂部）目前共 13 張：m1～m9（9 張）、體育組資料上傳（綠色）、競賽獎狀資料（金色，
