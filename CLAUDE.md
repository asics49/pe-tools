...
### 本機輔助腳本（2026-07-20 起已 commit 進 GitHub，僅本機/手動執行使用，不隨網站部署）
| 檔案 | 說明 |
|------|------|
| `_get_drive_ids.py` | 讀本機 Google Drive SQLite DB，列出「文件上傳」資料夾 ID |
| `_get_ref_ids.py` | 同上，列出「參考資料」資料夾 ID |
| `_patch_drive_urls.py` | 批次將 driveUrl 填入 index.html（89 筆，一次性腳本，已執行過） |
| `_patch_ref_urls.py` | 批次將 refUrl 填入 index.html（89 筆，一次性腳本，已執行過） |
| `_schema.py` | 資料結構定義 |
...
- **已於 2026-07-20 部署完成**：專案「體育班評鑑資料夾匯出歸檔」用**學校網域帳號
  `asics49@yocps.kh.edu.tw`** 執行與部署（跟現有上傳系統 Code.gs 慣用的 `asics49@gmail.com` 不同，
  但這個新工具用學校帳號才正確——因為「體育班評鑑模組平台」本來就是學校帳號的資料夾，用同一帳號
  部署才不會有 `DriveApp.getRootFolder()` 指到不同雲端硬碟的問題），index.html 卡片已連到部署網址
  `https://script.google.com/a/macros/yocps.kh.edu.tw/s/AKfycbyVHK0twqxFaVL3cGerGQcSuMAkSRFmXiE88f66CR6JuY2a3Z3LTkkAVNk6X7Wb-dfAWw/exec`；
  首次實測（複製到 1618 檔案/267 資料夾時）確認網頁介面、進度輪詢、背景觸發器接續皆正常運作，
  只是實際總耗時比程式註解樂觀估計的「十幾分鐘」慢很多——**每個檔案/資料夾操作都是一次 Drive API
  呼叫有網路延遲，加上每輪跑完要等 1 分鐘才排下一輪，2500+ 檔案抓 30~90 分鐘跑完較符合實際**
...
### 為何沒整合進評鑑總覽頁（重要決策，⚠️ 2026-07-18 後此問題已解決）
...
（依實際資料夾名稱分組才準）。**2026-07-18 晚間**：index.html 子標 id 全面重編為「大標-分組-序號」
格式並同步改名 Drive 資料夾後，兩邊編號已完全一致（僅壹的師資聘任 2 個資料夾例外，尚未實際拆分，
見上方待辦）——**若日後要把檔案大小檢查整合進總覽頁，這個原本的阻礙已經不存在，可重新評估**。

### 實際 Drive 資料夾結構（掃描依據，2026-07-20 更新路徑與編號）
```
體育班評鑑模組平台/                                                       ← 2026-07-20 從
│                                                                          🎾體育組\114學年度體育組\
│                                                                          🔥115年度體育班評鑑 搬移改名
├── 1.壹、設班現況/  2.貳、運作情形/  3.參、訓練績效/  4.肆、其他特色加分/   ← 大標層
│   └── 壹-一-3 體育班課程規劃/                                            ← 子標層（現與 index.html id 一致）
│       ├── (最外層檔案 = 要上傳的檔案，計入加總)
│       ├── 文件上傳/   ← 排除
│       └── 參考資料/   ← 排除
```
...
模組卡片列（頁面頂部）目前共 16 張：m1～m10 共 11 張（m3a/m3b 各一張）、體育組資料上傳（綠色）、競賽獎狀資料（金色，
連 114學年度競賽成績獎狀掃描 Drive 資料夾）、檔案大小檢查（紅色，連 file-size-check.html）、
評鑑資料匯出歸檔（2026-07-20 新增，深綠色，連 `_export_archive.gs` 所在的 Apps Script 專案編輯頁——
這是手動執行的腳本、無網頁介面，卡片只是方便快速找到專案入口）、
**工具使用說明（紫色，一律放最後一張）**。
...
- [x] **index.html 子標 id 全面重編＋Drive 資料夾同步改名**（2026-07-18 晚間）：30 個子標改成
      「大標-分組-序號」格式（`壹-一-1`／`貳-三-6`／`肆-1`），徹底消除既有跨大標 id 撞號；
      Google Drive 實際資料夾同步改名 30 個，壹的師資聘任 2 個資料夾因內部未依指標拆分，先合併
      改名、真正拆分留待評鑑結束後處理
- [x] **評鑑平台資料夾搬移**（2026-07-20）：使用者提早執行，從 `🎾體育組\114學年度體育組\
      🔥115年度體育班評鑑` 搬到 `我的雲端硬碟` 根目錄並改名為「體育班評鑑模組平台」（與原計畫
      目標位置不同，詳見下方章節）；驗證搬移前後檔案數（2512）、資料夾數（12/14/4）完全一致，
      driveUrl/refUrl 連結正常
...
- [ ] **評鑑平台資料夾歸檔＋清空**（搬移已提前於 2026-07-20 完成，見下方章節）：
      等本年度評鑑內容全部完成後，用 `_export_archive.gs`（2026-07-20 新增，詳見上方「本機輔助
      腳本」說明）伺服器端匯出整棵資料夾到歸檔位置，驗證無誤後再清空平台檔案（資料夾本身
      保留給下年度沿用）
...
## 評鑑平台資料夾搬移計畫（2026-07-16 評估定案，**2026-07-20 已執行搬移**）

### 執行結果（2026-07-20）
使用者提早執行搬移（未等評鑑內容全部完成），且**目標位置與原計畫不同**：
- 新路徑：`G:\我的雲端硬碟\體育班評鑑模組平台\`（原計畫是搬到 `🎾體育組\` 底下，實際改放在
  **我的雲端硬碟根目錄**，資料夾名稱從「🔥115年度體育班評鑑」改為「**體育班評鑑模組平台**」）
- 舊路徑 `🎾體育組\114學年度體育組\🔥115年度體育班評鑑` 現在**不存在**（已搬走）；同層有使用者
  自行手動備份的 `🔥115年度體育班評鑑 - 備份`（2276 個檔案，使用者確認是搬移前自己複製的安全網，
  非系統產生，**不要動它**）
- 搬移方式：使用者自己在 **drive.google.com 網頁版**操作（伺服器端原子操作，符合下方建議做法），
  未使用本機拖曳
- **未執行「歸檔」步驟**：因為評鑑內容還在進行中，這次只是單純搬移+改名，不是完整換年度流程，
  平台內容維持現狀（步驟 6「清空平台」尚未執行也不需要執行，等真正要換年度重複使用時才需要）
- **驗證結果**（搬移後）：本機路徑清點確認搬移前後完全一致——12/14/4 個資料夾（壹/貳/肆）、
  總計 2512 個檔案，無短少；抽查 2 筆 driveUrl/refUrl（`貳-三-1` 的文件上傳/參考資料資料夾）用
  Google Drive API 直接查詢，資料夾 ID 解析正常、內容物完整（refUrl 那筆用連接的 Drive API 帳號
  查詢一度回報「找不到」，改用本機路徑核對後確認只是**該 API 連線帳號的權限範圍問題**，資料夾
  實際完好，不是真的失效——**教訓：這個 Drive API 連接帳號對此資料夾樹的存取權限不完整/不穩定，
  之後驗證 driveUrl/refUrl 有效性優先用本機同步路徑核對內容物是否存在，比呼叫 API 查詢可靠**
- **本機路徑異動**：任何指到舊路徑 `G:\...\🎾體育組\114學年度體育組\🔥115年度體育班評鑑` 的捷徑/
  釘選都要重新指向 `G:\我的雲端硬碟\體育班評鑑模組平台`；`_get_drive_ids.py`／`_patch_drive_urls.py`
  沒有寫死此路徑（已確認 grep 不到），故不受影響，但這兩支腳本本來就是一次性工具、以後若要重跑
  仍要留意目前的新路徑

### 原始決策（供對照，已被上面實際執行結果取代）
使用者原本決定採方案 B：等本年度評鑑內容全部完成後再執行「歸檔 → 搬移」，評鑑衝刺期間不動資料夾結構。
...
> ✅ 搬移已於 2026-07-20 執行完成（新路徑 `G:\我的雲端硬碟\體育班評鑑模組平台\`），
> 資料夾與 ID 沿用平台既有的，步驟 1~5 大幅簡化：**只需在評鑑內容全部完成後，先歸檔當年度
> 30 個子標的內容（複製保存，未做過），再清空平台內當年度檔案即可**，index.html 的網址不用重打。
