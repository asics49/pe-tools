...
**發現但暫緩處理的既有 id 撞號問題**：全檔案 `sub.id` 在不同大標間有多組重複——`1-1-2`（壹的課程規劃
程序 vs 貳的籌措社會資源情形）、`3-1-1`／`3-1-2`（壹的訓練場地/器材 vs 貳的賽後補課/未達參賽基準）、
`4-1`／`4-2`（貳的性平/反霸凌 vs 肆的設班/運作特色）。因為「每筆文件完成追蹤」用
`sub.id::文件索引::文件名稱` 當 localStorage key（**不含大標題**），理論上有跨大標误标記完成的風險，
只是目前撞號雙方的文件名稱剛好都不同才没有實際觸發。這次重構刻意只用全新、不撞號的 id（`1a-*`／
`1b-*`）處理有觸碰到的子標，**其餘既有撞號留待未來評鑑空檔專門處理**（修正會連帶重置該子標下所有
文件的「已完成」勾選狀態，需與使用者確認時機）。「子標層級備忘/已完成」用 `大標題::子標id` 當 key
（含大標題）不受此問題影響
